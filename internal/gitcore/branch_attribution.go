...
func (r *Repository) commitBranchAttribution() map[Hash]branchAttribution {
	r.mu.RLock()
	commits := r.commitsMap()
	refs := make(map[string]Hash, len(r.refs))
	...
	return buildCommitBranchAttribution(commits, refs, headRef)
}
...
func buildCommitBranchAttribution(
	...
	headRef string,
) map[Hash]branchAttribution {
	if len(commits) == 0 {
		return map[Hash]branchAttribution{}
	}

	out := make(map[Hash]branchAttribution)
...
func collectBranchSeeds(refs map[string]Hash, headRef string) []branchSeed {
...
func cloneCommitWithBranchAttribution(commit *Commit, attribution branchAttribution) *Commit {
	if commit == nil {
		return nil
	}
