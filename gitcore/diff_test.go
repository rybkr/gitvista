...
func TestBuildHunksMergesOverlappingContextWindows(t *testing.T) {
	oldLines := []string{"a", "b", "c", "d", "e"}
	newLines := []string{"b", "c", "d", "X", "e"}
	edits := []edit{
		{Type: editDelete, OldLine: 0},
		{Type: editKeep, OldLine: 1, NewLine: 0},
		{Type: editKeep, OldLine: 2, NewLine: 1},
		{Type: editKeep, OldLine: 3, NewLine: 2},
		{Type: editInsert, NewLine: 3},
		{Type: editKeep, OldLine: 4, NewLine: 4},
	}

	hunks := buildHunks(oldLines, newLines, edits, 2)
	if len(hunks) != 1 {
		t.Fatalf("len(buildHunks()) = %d, want 1 merged hunk", len(hunks))
	}
	if len(hunks[0].Lines) != 6 {
		t.Fatalf("len(hunks[0].Lines) = %d, want 6", len(hunks[0].Lines))
	}
	if hunks[0].Lines[2].Content != "c" || hunks[0].Lines[3].Content != "d" {
		t.Fatalf("merged hunk context = %#v, want overlapping context preserved once", hunks[0].Lines)
	}
	if hunks[0].Lines[4].Type != LineTypeAddition {
		t.Fatalf("hunk lines[4] = %#v, want addition after merged context", hunks[0].Lines[4])
	}
}

func TestBuildHunksSplitsSeparateContextWindows(t *testing.T) {
	oldLines := []string{"a", "b", "c", "d", "e", "f"}
	newLines := []string{"A", "b", "c", "d", "E", "f"}
	edits := []edit{
		{Type: editDelete, OldLine: 0},
		{Type: editInsert, NewLine: 0},
		{Type: editKeep, OldLine: 1, NewLine: 1},
		{Type: editKeep, OldLine: 2, NewLine: 2},
		{Type: editKeep, OldLine: 3, NewLine: 3},
		{Type: editDelete, OldLine: 4},
		{Type: editInsert, NewLine: 4},
		{Type: editKeep, OldLine: 5, NewLine: 5},
	}

	hunks := buildHunks(oldLines, newLines, edits, 1)
	if len(hunks) != 2 {
		t.Fatalf("len(buildHunks()) = %d, want 2 separate hunks", len(hunks))
	}
	if hunks[0].OldStart != 1 || hunks[0].NewStart != 0 {
		t.Fatalf("first hunk start = -%d +%d, want -1 +0", hunks[0].OldStart, hunks[0].NewStart)
	}
	if hunks[1].OldStart != 4 || hunks[1].NewStart != 4 {
		t.Fatalf("second hunk start = -%d +%d, want -4 +4", hunks[1].OldStart, hunks[1].NewStart)
	}
	if hunks[0].Lines[len(hunks[0].Lines)-1].Content != "b" {
		t.Fatalf("first hunk trailing context = %#v, want trailing context for b", hunks[0].Lines)
	}
	if hunks[1].Lines[0].Content != "d" {
		t.Fatalf("second hunk leading context = %#v, want leading context for d", hunks[1].Lines)
	}
}

func TestBuildHunkDeleteOnly(t *testing.T) {
	hunk := buildHunk([]string{"gone"}, nil, []edit{{Type: editDelete, OldLine: 0}})

	if hunk.OldStart != 1 || hunk.NewStart != 0 {
		t.Fatalf("hunk start = -%d +%d, want -1 +0", hunk.OldStart, hunk.NewStart)
	}
	if hunk.OldLines != 1 || hunk.NewLines != 0 {
		t.Fatalf("hunk sizes = -%d +%d, want -1 +0", hunk.OldLines, hunk.NewLines)
	}
	if len(hunk.Lines) != 1 || hunk.Lines[0].Type != LineTypeDeletion {
		t.Fatalf("hunk lines = %#v, want one deletion", hunk.Lines)
	}
}

func TestBuildHunkInsertOnly(t *testing.T) {
	hunk := buildHunk(nil, []string{"added"}, []edit{{Type: editInsert, NewLine: 0}})

	if hunk.OldStart != 0 || hunk.NewStart != 1 {
		t.Fatalf("hunk start = -%d +%d, want -0 +1", hunk.OldStart, hunk.NewStart)
	}
	if hunk.OldLines != 0 || hunk.NewLines != 1 {
		t.Fatalf("hunk sizes = -%d +%d, want -0 +1", hunk.OldLines, hunk.NewLines)
	}
	if len(hunk.Lines) != 1 || hunk.Lines[0].Type != LineTypeAddition {
		t.Fatalf("hunk lines = %#v, want one addition", hunk.Lines)
	}
}

func TestBuildHunkUnknownEditTypeProducesEmptyHunk(t *testing.T) {
	hunk := buildHunk(nil, nil, []edit{{Type: editType(99)}})

	if hunk.OldStart != 0 || hunk.NewStart != 0 {
		t.Fatalf("hunk start = -%d +%d, want -0 +0", hunk.OldStart, hunk.NewStart)
	}
	if hunk.OldLines != 0 || hunk.NewLines != 0 {
		t.Fatalf("hunk sizes = -%d +%d, want -0 +0", hunk.OldLines, hunk.NewLines)
	}
	if len(hunk.Lines) != 0 {
		t.Fatalf("hunk lines = %#v, want empty", hunk.Lines)
	}
}
