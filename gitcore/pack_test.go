...
func TestReadPackObject_DeltaDepthExceeded(t *testing.T) {
	...
	noopResolver := func(id Hash, depth int) ([]byte, ObjectType, error) {
		return nil, 0, fmt.Errorf("should not be called")
	}

	_, _, err := readPackObjectData(bytes.NewReader(buf.Bytes()), noopResolver, maxDeltaDepth+1)
...
func TestReadPackObject_DeltaChainWithinLimit(t *testing.T) {
	...
	firstByte := (byte(ObjectTypeOfsDelta) << 4) | byte(size&0x0F)
	size >>= 4
	if size > 0 {
		firstByte |= 0x80
	}
	...
	data, objType, err := readPackObjectData(reader, noopResolver, 0)
	if err != nil {
		t.Fatalf("readPackObjectData failed: %v", err)
	}
	if objType != ObjectTypeBlob {
		t.Errorf("expected type %d (blob), got %d", ObjectTypeBlob, objType)
	}
